...
st.set_page_config(page_title="EXP 계산기", page_icon="🗡️")

st.title("EXP 계산기 by 래유")
...
with col1:
    cur_lv = st.number_input("현재 레벨", min_value=1, max_value=199, value=1)
    cur_pct = st.number_input("현재 경험치 (%)", min_value=0, max_value=99, value=0)

with col2:
    tar_lv = st.number_input("목표 레벨", min_value=2, max_value=200, value=2)
    rate = st.number_input("10분당 획득 경험치", min_value=0, value=0)

if st.button("계산하기"):
    if cur_lv >= tar_lv:
        st.error("목표 레벨은 현재 레벨보다 높아야 합니다.")
    elif rate <= 0:
        st.error("경험치 획득량은 0보다 커야 합니다.")
    else:
        ...
        st.success(f"필요 경험치: **{int(final_needed):,}**")
        st.info(f"예상 소요 시간: **{h}시간 {m}분**")

st.divider()

# --- Timer Section ---
st.header(" 10분 사냥 타이머")
if 'remain' not in st.session_state:
    st.session_state.remain = 600
if 'run' not in st.session_state:
    st.session_state.run = False

timer_text = st.empty()
mins, secs = divmod(st.session_state.remain, 60)
timer_text.markdown(f"###  {mins:02d}:{secs:02d}")

t_col1, t_col2 = st.columns(2)
if t_col1.button("시작/정지"):
    st.session_state.run = not st.session_state.run

if t_col2.button("초기화"):
    st.session_state.remain = 600
    st.session_state.run = False
    st.rerun()

if st.session_state.run and st.session_state.remain > 0:
    while st.session_state.remain > 0 and st.session_state.run:
        time.sleep(1)
        st.session_state.remain -= 1
        mins, secs = divmod(st.session_state.remain, 60)
        timer_text.markdown(f"###  {mins:02d}:{secs:02d}")
        if st.session_state.remain == 0:
            st.balloons()
            st.success("10분 사냥이 끝났습니다!")
            st.session_state.run = False
            break
...
# --- Cheer Button ---
if st.button("화이팅", use_container_width=True):
    st.toast("시프 화이팅! ")
    st.snow()